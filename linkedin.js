...
async function login(page, user, password) {
    ...
    // Insert user
    await PupUtils.waitForFocusAndType(page, userSelector, user, "User");
    // Insert password
    await PupUtils.waitForFocusAndType(page, passwordSelector, password, "Password");
    // Submit to login
    await PupUtils.waitForFocusAndClick(page, submitSelector, "Submit");
}
...
async function publishPost(page, content, imagesPaths = []) {
    ...
    await PupUtils.waitForFocusAndClick(page, createPostSelector, "Create post");

    // Upload image if the imagesPaths paramter is not empty
    if (imagesPaths.length !== 0) {
        const [fileChooser] = await Promise.all([
            page.waitForFileChooser(),
            PupUtils.waitForFocusAndClick(page, uploadImageSelector, "Upload Image", true)
        ]);
